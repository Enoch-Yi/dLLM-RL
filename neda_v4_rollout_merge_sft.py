#!/usr/bin/env python3
"""Fail-closed merge of SFT-initialized, SFT-disjoint ALFWorld shards."""
...
PROTOCOL_FIELDS = (
    ...
    "training_game_pool_contract",
    "total_train_games",
    "excluded_sorted_prefix_games",
    "eligible_rl_games",
    "excluded_game_ids_sha256",
    "eligible_game_ids_sha256",
)
...
def merge_rollouts(
    ...
    if (
        reference.get("training_game_pool_contract")
        != "neda-sft-disjoint-alfworld-train-v1"
        or int(reference.get("total_train_games", -1)) != 3553
        or int(reference.get("excluded_sorted_prefix_games", -1)) != 1000
        or int(reference.get("eligible_rl_games", -1)) != 2553
    ):
        raise ValueError("SFT-disjoint ALFWorld training-pool contract drift")
    ...
    selected_game_ids = sorted({str(row["game_id"]) for row in rows})
    if len(selected_game_ids) != int(expected_games):
        raise ValueError("merged rollout selected-game coverage drift")
    ...
    manifest = {
        ...
        "selected_game_ids_sha256": sha256_json(selected_game_ids),
